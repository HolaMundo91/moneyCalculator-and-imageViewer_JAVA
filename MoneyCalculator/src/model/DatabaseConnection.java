...
public class DatabaseConnection implements CurrencySetLoader{
    ...
    private ExchangeRate processExchangeRate(ResultSet resultSet, Currency fromCurrency, Currency toCurrency) throws SQLException {
        return new ExchangeRate( fromCurrency, toCurrency, Double.parseDouble(resultSet.getString("rate")) );
    }
    
    public ExchangeRate getExchangeRate(Currency fromCurrency, Currency toCurrency) {
        String query = "SELECT * FROM exchangeRate WHERE fromCurrency = '"+ fromCurrency +"' AND toCurrency = '"+ toCurrency +"'";
        try {
            return processExchangeRate( connection.createStatement().executeQuery(query),fromCurrency,toCurrency );
        } catch (SQLException exception) {
            return null;
        }
    }
